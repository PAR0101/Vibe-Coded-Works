"""
get_ffmpeg.py — Downloads ffmpeg.exe into the YTMusicPlayer folder.
Run once: python get_ffmpeg.py
"""
...
def download_ffmpeg():
    if os.path.exists(DEST):
        print(f"ffmpeg already exists at: {DEST}"); return
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            for name in zf.namelist():
                if name.endswith("/bin/ffmpeg.exe"):
                    ...
                    print(f"ffmpeg.exe saved to: {DEST}"); break
            else:
                print("ERROR: ffmpeg.exe not found inside zip"); sys.exit(1)
    finally:
