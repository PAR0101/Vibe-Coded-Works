...
Audio engine using:
  ...
  - ffmpeg  → decode to raw PCM (subprocess)
  - sounddevice → play PCM chunks in real time

No pygame. No VLC. No temp files.
Requires: pip install yt-dlp sounddevice numpy
Requires: ffmpeg.exe in YTMusicPlayer folder (run get_ffmpeg.py once)
"""
...
def _find_ffmpeg() -> Optional[str]:
    # Same folder as main.py (project root)
...
try:
    ...
    # Test that audio device is available
...
CHUNK_SIZE   = SAMPLE_RATE // 10 * BYTES_PER_FRAME   # 100ms chunks
...
class FfmpegPlayer:
    """
    Streams a URL through ffmpeg → sounddevice.
    Runs in a background thread. Thread-safe pause/stop.
    """

    def __init__(self, url: str, volume: float = 0.7,
...
class AudioEngine(QObject):
    ...
    def __init__(self, parent=None):
        ...
        if not FFMPEG:
            print("[AudioEngine] WARNING: ffmpeg not found. Run get_ffmpeg.py first.")
        if not _SD_OK:
            print("[AudioEngine] WARNING: sounddevice not working.")
    ...
    def load_and_play(self, track: TrackInfo):
        ...
        threading.Thread(target=self._resolve_worker, args=(track,), daemon=True).start()
    ...
    def seek(self, ms: int):
        # Seek by restarting ffmpeg with -ss offset
    ...
    def set_loop(self, on: bool):
        self._looping = on
        print(f"[AudioEngine] Loop: {'on' if on else 'off'}")
    ...
    def _search_worker(self, query, max_results):
        try:
            ...
            self.error_occurred.emit(f"Search error: {e}")
    ...
    def _resolve_worker(self, track: TrackInfo, seek_ms: int = 0):
        ...
        try:
            ...
            self._pending_url    = stream_url
            self._pending_dur    = duration
            self._pending_seek   = seek_ms

            from PyQt5.QtCore import QMetaObject, Qt as QtNS
            QMetaObject.invokeMethod(self, "_qt_play", QtNS.QueuedConnection)

        except Exception as e:
            self._loading = False
            self.error_occurred.emit(f"Resolve error: {e}")
    ...
    def _qt_play(self):
        ...
        if not url:
            self.error_occurred.emit("Could not get stream URL.")
            self.state_changed.emit("error")
            return

        if not FFMPEG:
            self.error_occurred.emit(
                "ffmpeg not found.\nRun: python get_ffmpeg.py"
            )
            self.state_changed.emit("error")
            return

        if not _SD_OK:
            self.error_occurred.emit("sounddevice not working. Try: pip install sounddevice")
            self.state_changed.emit("error")
            return
        ...
        player = FfmpegPlayer(
            ...
            on_position=self._cb_position,
            on_end=self._cb_end,
            on_error=self._cb_error,
        )
        self._player = player
        self._pos_ms = seek_ms
        player._pos_ms = seek_ms   # initialise counter at seek offset
        player.start()

        self._state = "playing"
        self.state_changed.emit("playing")

    # Callbacks from FfmpegPlayer thread
    ...
    def _cb_end(self):
        ...
        self.state_changed.emit("stopped")
        from PyQt5.QtCore import QMetaObject, Qt as QtNS
        QMetaObject.invokeMethod(self, "_advance", QtNS.QueuedConnection)
    ...
    def _advance(self):
        if self._looping and self._current_track:
            ...
            t.stream_url = ""   # force re-resolve
            self.load_and_play(t)
